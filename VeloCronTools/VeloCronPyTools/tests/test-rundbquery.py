...
class TestQuery(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if __hostname__.find('plus') == 0:
            self.cmd = ['rdbt', '-n', '137259']
        else:
            self.cmd = ['cat', __test_dir__ + '/rdbt_single_run.txt']

    def test_good_run_number(self):
        query = RunDBQuery(self.good_runno, self.cmd)
        self.assertTrue(query.get_valid_runs(1800))

    def test_good_run_list(self):
        query = RunDBQuery(self.good_runlist, self.cmd)
        self.assertTrue(query.get_valid_runs(1800))

    def test_bad_run_number(self):
        query = RunDBQuery(self.bad_runno, self.cmd)
