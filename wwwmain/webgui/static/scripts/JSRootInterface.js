...
var source_dir = "/static/";//  "http://root.cern.ch/js/";
var gFile;
...
//the code for displaying right graph matrix in tabs
var reportTwo = false;
var reportThree = false; 

//defining an array
var histo = ['h1','h2','h3','h4','h5','h6','h7','h8'];
var count = 0;
...
/*   showObject('h1',1);
     ...
     showObject('h6',1);
     showObject('h7',1);
     showObject('h8',1);*/ //old implementation

     for(count=0;count<keys.length;count++){
         console.log("keys = "+keys[count]['name']);
         showObject(keys[count]['name'],1);	
	}
}
...
//    console.log("obj_name, cycle = "+obj_name+" "+cycle); //for the debug purpose
    setTimeout(function() {gFile.ReadObject(obj_name, cycle);},(100)); //seting up timeout so that all graphs load everytime. This is due to the assyncronous code.
...
//   console.log("idx is ="+idx); //for the debug purpose
//   console.log("reportTwo ="+reportTwo); //for the debug purpose
//   console.log("reportThree ="+reportThree); //for the debug purpose
   
   //modified lines to suit our requirements of the layout (2x4)
   ...
   //execution performance test for Sensor View tab
/*   if(idx==7){
...
//      console.log("AsserPrereq...Loading scripts.."); 
...
//	    showObject('h2',1);
         ...
         console.log("after ...addClass and gFile is "+gFile);
      }) }) }) }) }) }) }) }) }) }) });// console.log("after loadScript in Assert..., before return true");
       return true;	
   }
//   console.log("oops, we return false");
   return false;
};

function ReadFile() {
  
   console.log("entering READFILE");
   ...
   if (typeof ActiveXObject == "function") { // Windows
     ...
     if (AssertPrerequisites()) {/*console.log("We returned to ReadFile()");*/return;}
